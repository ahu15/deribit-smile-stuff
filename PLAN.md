...
6. **M3.6 — Vol-time / working-day calendar.** `backend/vol_time.py` (per-day weighting, holiday set, `vol_yte` / `cal_yte` conversions adapted from internal utilities — ms-int timestamps throughout, `holiday_weights: dict[date, float]` rather than a single sentinel weight so each holiday carries its own assumption). HTTP routes `/api/calendar` (GET, POST) and `/api/calendar/recalibrate` (POST) on the FastAPI side; tabs never hit these directly — frontend access goes through a new dual-mode `calendarService` (`worker/calendarService.ts`) exposing `getCalendar()`, `putCalendar(c)`, `recalibrate()` as oracle-routed one-shots, plus a `calendarStream()` conversation that yields the active `Calendar` object on subscribe and re-yields whenever a put or recalibrate completes (HRT principles 1, 2, 3). New `VolCalendar` widget for manual entry: holiday list table with date / name / weight + delete, weekday-weight inputs, live diagnostics rail (`total vol days / yr`, per-expiry `dte` vs `dte_wkg` + ratio). Holiday weights default to a small `frontend/src/shared/calendarPresets.ts` JSON of preset values (`{label: "Full holiday", weight: 0.1}`, `{label: "Half day", weight: 0.5}`, `{label: "Custom", weight: null}`) — user picks from the list every time per spec. Single shared calendar across BTC + ETH for now (per-venue/per-currency calendars deferred until M6 Bloomberg makes them necessary). Persisted to `localStorage` (global, NOT per Dockview profile) and mirrored to the backend on boot via `calendarService.putCalendar`. **Calendar revision** is a hash of `(holiday_weights, sat_weight, sun_weight)` only — name edits do NOT bump the revision. **No automatic refit happens on edit.** A `recalibrate` button on `VolCalendar` is the single trigger that walks every cached fit (live `(currency, expiry, methodology, ts_method, snapshot_ts)` engine cache plus the M3.9 bucketed-fit cache for past curves) and recomputes under the current calendar revision — recalibrating both backward (every bucketed historic fit) and forward (every snapshot from this point on uses the new revision in its cache key). The button surfaces a confirmation count ("recalibrate 142 cached fits across 14 expiries × 8 methodologies × 24h history?") before running. After recalibrate completes, every open `subscribe_smile` / `subscribe_termstructure` / `subscribe_methodology_buckets` conversation re-emits the affected envelopes with the new `calendar_rev` field — clients learn about the change through the conversations they already subscribe to (HRT principle 3), not through a bus replay or polling. Conversation IDs and dedup keys are unchanged across recalibrate so open charts don't churn (HRT principle 6). Every existing `t_years` site routes through `vol_yte`; default calendar matches today's hard-coded behavior so M3.5 widgets are byte-identical until the user edits something and hits recalibrate.
7. **M3.7 — Methodology engine refactor.** Backend `Calibrator` + `CurveBuilder` ABCs (`backend/calibration/`, `backend/curves/`). `MethodologyRegistry` enumerating presets along three axes (`family: sabr`, `freeze: none | alpha-from-ts | volvol-and-alpha-from-ts`, `weights: uniform | atm-manual | bidask-spread | bidask-spread-sma`). Each preset declares `requires_ts: bool` and `time_basis: "cal" | "wkg"`. **Two-layer dedup, both honoring HRT principle 1:** (a) backend engine cache `{(currency, expiry, methodology, ts_method, snapshot_ts, calendar_rev): FitResult}` lives in the FastAPI singleton inside `DeribitAdapter` so each unique compute happens at most once per chain poll; (b) frontend SharedWorker oracle refcount-dedups WS conversations via `acquireSharedStream` keyed on `(currency, expiry, methodology, ts_method)` — `calendar_rev` is intentionally NOT in the conversation key (it travels on the envelope) so a recalibrate doesn't churn open subscriptions. Naive SABR moves under the engine as `methodology="sabr-naive"`. WS subscription extended to `subscribe_smile {currency, expiry, methodology, termStructure}`. New oracle-routed one-shot service `methodologyCatalog` (`worker/methodologyService.ts`) wraps `GET /api/methodologies` (label, family, requires_ts, time_basis, param schema) — fetched once at boot, never streamed, refcount-shared across tabs (HRT principles 1, 2). `FitResult` is a tagged union of plain data only — `{kind: "sabr", params: {...}, fitted_iv: number[], weights_used: number[], frozen: {param, value, source}[], residual_rms, weighted_residual_rms, t_years_cal, t_years_wkg, calendar_rev}` — no methods cross the port (HRT principle 4). Any frontend `evaluate(strikes, params)` lives in a `frontend/src/calibration/` table keyed by `family` and is reconstructed locally from the shipped params; the backend ships pre-sampled `fitted_iv` for chart rendering. `SmileChart` toolbar gains a methodology dropdown (single option for now) plus a TS dropdown (disabled). **No user-visible behavior change from M3.5**, just new plumbing.
8. **M3.8 — DMR term-structure curves + dependent smile presets.** `backend/curves/dmr_util.py` (the existing `dmr_util_functions.py` math, ported with the `data_structures` constant lifted into `backend/calibration/constants.py`). Two `CurveBuilder`s: `ts_alpha_dmr` (consumes naive-SABR α(T) cache, runs `fit_double_mean_reversion`, emits `TermStructureSnapshot` with `alpha_at_T` / `atm_vol_at_T` / `fwd_var_at_T` callables) and `ts_atm_linear_dmr` (consumes ATM mark-IV per expiry directly from the chain). `backend/calibration/rho_smr.py` (the existing `fit_rho_smr` math) plus a sibling `volvol_smr.py` for ν-smoothing. Note `TermStructureSnapshot`'s callables are *backend-side* — the WS envelope ships pre-sampled `(t_years_grid, atm_vol_grid, alpha_grid, fwd_var_grid)` arrays plus the fitted `params` so the frontend `TermStructureChart` reconstructs whatever curve it needs from plain data (HRT principle 4). Smile presets that consume the TS bundle: `sabr-alpha-frozen` (α pinned from TS, ρ/ν free), `sabr-volvol-and-alpha-frozen` (α pinned from TS, ν pinned from `volvol_smr`, ρ free), each variant available in `uniform`, `atm-manual`, `bidask-spread`, `bidask-spread-sma` weighting flavors. **New WS conversation envelope `subscribe_termstructure {currency, method}`** emitting `termstructure_snapshot` envelopes on every chain poll, paired with a new dual-mode service `worker/termstructureService.ts` (oracle-mode runs the conversation, client-mode is a transparent `subscribeRemote` proxy yielding `TermStructureSnapshot` to consumers via `for await...of` — HRT principles 2, 3). Oracle dedup key for the term-structure stream is `(currency, method)`; `calendar_rev` rides the envelope, not the key. New `TermStructureChart` widget: SVG plot, methodology dropdown, currency picker, x-axis toggle (cal/wkg), y-axis toggle (`σ_atm` / `α` / `forward variance`), parameter readout (`v_final`, `v0`, `w0`, `λ_short`, `λ_long`, `ρ_∞`, `ρ_0`, `λ_ρ`), RMSE readout, frozen-curve overlay (one-shot `historicTermStructure(currency, method, asOfMs)` service mirroring `historicSmile`'s pattern). `SmileChart` TS dropdown becomes live; the methodology dropdown enforces the `requires_ts` constraint at config-form level.
9. **M3.9 — History overlays + cross-methodology comparison + bucketed fit cache.** Backend bucketed fit cache `{(currency, expiry, methodology, ts_method, bucket_ts, calendar_rev): FitResult}` keyed at hourly boundaries for the last 24h, seeded on first subscription per `(currency, expiry, methodology, ts_method)` from the `HistoryStore` chain replay. **This cache is built here for the M4.5 `AnalysisService` to consume later** — `fitHistory` becomes a lookup over this cache rather than its own bucketing pass. Frontend exposure follows the existing conversation pattern (HRT principle 3): new WS envelope `subscribe_methodology_buckets {currency, expiry, methodology, ts_method, lookback_ms}` emits a `buckets_snapshot` (the cached fits within the lookback window) followed by `bucket_append` envelopes when a new hour-bucket lands at the head, plus `buckets_invalidated` envelopes on calendar_rev change so subscribers refit their views without re-subscribing. Same shape as `subscribe_history` / `subscribe_aggregate`. Wrapped in a dual-mode `worker/bucketsService.ts` (HRT principle 2) with refcount-shared streams via `acquireSharedStream` keyed on `(currency, expiry, methodology, ts_method, lookback_ms)`. Hourly-snapshot overlays land in both `SmileChart` and `TermStructureChart`: settings-panel "show N hourly snapshots" multi-select renders the last N curves color-faded by age, sourced from `bucketsService`. Cross-methodology comparison mode in both widgets: settings-panel multi-select opens additional `subscribe_smile` / `subscribe_termstructure` conversations under different methodology keys — oracle dedup ensures other widgets at the same key share the stream. Calendar-revision invalidation hooks into the bucketed cache: recalibrate button per M3.6 walks every key whose `calendar_rev` is stale, recomputes, then emits `bucket_append` (or `buckets_snapshot` resend) envelopes to every open subscription so charts redraw without remount (HRT principle 6).
10. **M3.95 — `ModelHealth` widget.** Single tabbed widget with three internal tabs and **persistent per-tab state** (current methodology selection, expiry filter, time-window, scroll position, etc. live in the widget config so switching tabs is seamless and a tab restored from a saved profile remembers its last-viewed sub-state). Tab 1 — RMSE matrix (heatmap rows: methodology presets, columns: expiries; row-total weighted-average column on the right; surface-wide cell top-left) plus a tenor-breakdown line chart (one line per methodology, x = `yte`, y = `RMSE vol points`). Tab 2 — Parameter stability (small-multiples per fitted parameter per methodology, last 24h sourced from M3.9's bucketed cache; per-panel std-dev readout). Tab 3 — Vol-time diagnostics (calendar-vs-wkg side-by-side residual delta per expiry per methodology; wkg-vol residual heatmap bucketed by holidays-in-life: 0 / 1 / 2+). Toolbar above the tabs: methodology multi-select to narrow the comparison set. No new backend compute — every panel reads from the methodology engine's caches and aggregates client-side.
...
- **Methodologies factor along three axes**, not as N hand-rolled subclasses. Smile presets are `(family, freeze, weights)` triples; term-structure builders are `(method, dependencies)` pairs. The methodology registry enumerates the named combinations the user picks from in dropdowns.
...
- **Vol-time / wkg-time runs through every layer.** `backend/vol_time.py` exposes `vol_yte(expiry_ms, as_of_ms, calendar)` and `cal_yte(expiry_ms, as_of_ms)`. Calibrators take `t_years_wkg`; `dmr_util` already accepts arbitrary `yte` arrays, so the wrappers in `backend/curves/` do the cal→wkg conversion at the boundary. Every fit envelope ships both `t_years_cal` and `t_years_wkg` so the frontend axis-toggle is free. Each preset declares `time_basis: "cal" | "wkg"` and the pricer respects it per leg.
...
- **No automatic refit on calendar edit.** A `recalibrate` button on `VolCalendar` is the single trigger. On click: walk every cached fit (live engine cache + M3.9 bucketed cache), compute the diff vs current revision, surface a confirmation count ("recalibrate 142 cached fits across 14 expiries × 8 methodologies × 24h history?"), and on confirm recompute every stale entry. This handles both directions — every previously cached historic-bucket fit is refitted under the new calendar, and every snapshot from this point forward uses the new revision in its cache key. Frozen-overlay subscribers get fresh envelopes carrying the new `cal rev: N` annotation in the toolbar.
...
- Whether `ModelHealth`'s vol-time tab should also show a "calendar diff" view comparing residuals at calendar revision N vs N-1 (would let the user A/B-test holiday weight tweaks). Probably not v1 — the recalibrate-and-look loop covers it.
